...
function CheckoutPage() {
    ...
    const handleIncrease = (product_id: string) => {
        ...
        if (orderString) {
            ...
            if (orderCheckout) {
                console.log("check order quantity ", orderProduct)
                setOrderCheckout({
                    ...orderCheckout,
    ...
    const handleAddProduct = (product: Product) => {
        ...
        if (orderString) {
            ...
            console.log("check prduc tn ", checkProductInOrder)
            if (checkProductInOrder) {
                orderProduct = orderProduct.map(item =>
                    item.id === checkProductInOrder.id ?
                        { ...checkProductInOrder, quantity: (checkProductInOrder.quantity + 1) }
    ...
    const handleConfirmOrder = async () => {
        ...
        console.log("check create order ::: ", dataOrder.data?.id)
    ...
    return (
        <ContainerLayout isSidebar={false} isFooter={false}  >
            <div className="max-w-[1024px] w-full mx-auto mb-[150px] lg:mb-0">
                <div className="pb-3">
                ...
                <div className="w-full min-h-screen">
                    <div className='w-full flex flex-col lg:flex-row gap-3'>
                                            ...
                                            {productAddition && productAddition.results.length > 0 && productAddition.results.map(
                                                (product) => (
                                                    <Link href={`${DETAIL_PRODUCT_URL}/${product.id}`}>
                                                        <CardProductFull
                                                            className='!max-w-[260px] md:!max-w-[194px] !min-w-[194px]'
                                                            key={product.id}
                                                            id={product.id}
                                                            product_thumbnail={product.product_thumbnail}
                                                            product_name={product.product_name}
                        ...
                        <div className="fixed bottom-0 left-0 lg:sticky lg:top-2 max-w-full lg:max-w-[372px] lg:h-[491px] w-full space-y-3">
                            {/* CHECKOUT */}
                            {!show && <div className="absolute -top-[15%] left-[50%] transform -translate-x-1/2 transition-opacity duration-300 ease-in-out">
                            ...
                            <div className="bg-white px-4 py-0 lg:py-4 rounded-lg space-y-2 transition-all duration-300 ease-in-out"
                                ...
                                <button disabled={!isOrderAble} className="bg-gradient disabled:opacity-[0.5] disabled:cursor-none cursor-pointer mt-4 w-full text-white py-2 rounded-full hover:bg-red-700 transition-colors duration-300"
